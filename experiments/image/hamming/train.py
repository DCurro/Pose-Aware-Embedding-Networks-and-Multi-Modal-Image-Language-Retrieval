...
class ThinSlicingTrainset(torch.utils.data.dataset.Dataset):
    def __init__(self, far_max=None):
        ...
        self.nearfarsplit = np.load(nearfarsplit_train_file)
...
class ThinSlicingValset(torch.utils.data.dataset.Dataset):
    def __init__(self):
        ...
        self.nearfarsplit = np.load(nearfarsplit_val_file)
    ...
    def load_batch(self, index):
        ...
        near_distance = self.nearfarsplit[index]
        selection = [0] + shuffled(range(1,near_distance))[0:5] + shuffled(range(near_distance,self.nearest_indices.shape[1]))[0:105]
